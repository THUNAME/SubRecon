...
int main(int argc, char *argv[]) {
    ...
    if (argc != 8) {
        printf("Usage: %s <interface_name> <source_mac> <source_ip> <gateway_mac> <input_filename> <output_filename_subnet> <output_filename_router>\n", argv[0]);
        return 1;
    }
    ...
    processAndParsePrefix(input_filename);
    ...
    file_subnet = fopen(output_filename_subnet, "w");
    if (file_subnet == NULL) {
    ...
    while (prefix_table_size != 0) {
        ...
        memset(next_table, 0, sizeof(PrefixInfo) * MAX_PREFIX_TABLE_SIZE);
        ...
        for (uint64_t i = 0; i < last_prefix_table_size; i++) {
            // Print prefixes that meet criteria
            if ((cur_table[i].unique_ifaces == 1 && cur_table[i].prefix_length == 64) ||
                (cur_table[i].unique_ifaces == 1 && cur_table[i].total_replies > 0.9 * getValueByLength(cur_table[i].prefix_length))) {

                fprintf(file_subnet, "%04lx:%04lx:%04lx:%04lx::/%lu\n",
            ...
            // Process prefix refinement if needed
            if ((cur_table[i].unique_ifaces > 1) ||
                (cur_table[i].unique_ifaces == 1 && cur_table[i].total_replies <= 0.9 * getValueByLength(cur_table[i].prefix_length))) {

                if (cur_table[i].prefix_length >= 64) continue;
